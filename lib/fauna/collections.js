...
import { createRowsIndex, createBySlugIndex } from "./indexes";
...
export async function createCollection(name) {
    ...
    await createBySlugIndex(collectionName);
    return collection;
}
...
export async function checkCollection(name) {
    const collectionName = `${process.env.NEXT_PUBLIC_DB_PREFIX}_${name}`;
    const bool = await faunaClient.query(
